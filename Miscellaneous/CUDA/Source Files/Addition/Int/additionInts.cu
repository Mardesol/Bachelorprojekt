...
const char* executeChosenKernel(int KernelNumToPerform, int *device_M1, int *device_M2, int *device_M3, int MRows, int MCols, Timer timer)
{
    dim3 blockDim(32, 32);
...
int main(int argc, char *argv[])
{
    ...
    size_t memorySize = MRows * MCols * sizeof(int);
    ...
    MatrixI MCPU = createMatrixInts(MRows, MCols);
    ...
    sprintf(fileName, "Test/Addition_%s_Ints_Runtime_Matrix_Size_%dx%d.csv", kernelName, MRows, MCols);
